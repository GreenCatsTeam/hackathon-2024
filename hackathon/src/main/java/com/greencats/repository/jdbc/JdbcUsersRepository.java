...
@Repository @RequiredArgsConstructor public class JdbcUsersRepository implements UsersRepository {
    ...
    @Override public Long usersIdDelete(Long id) {
        int affectedRows =
            client.sql("UPDATE users SET is_deleted = TRUE WHERE user_id = :id").param("id", id).update();
    ...
    @Override public Long usersIdPut(Long id, UserEditInfo userEditInfo) {
        int affectedRows = client.sql("UPDATE users SET " +
                "first_name =:first_name, last_name =:last_name, email =:email, password =:password, role =:role, organization =:organization " +
                "WHERE user_id =:id").param("first_name", userEditInfo.first_name())
            .param("last_name", userEditInfo.last_name()).param(EMAIL_FIELD, userEditInfo.email())
            .param(PASSWORD_FIELD, userEditInfo.password()).param("role", userEditInfo.role())
            .param("organization", userEditInfo.organization()).param("id", id).update();
    ...
    @Override public UserCredentials findByEmail(String email) {
        return client.sql("SELECT email, password, role FROM users WHERE email = :email").param(EMAIL_FIELD, email)
            .query(UserCredentials.class).optional().orElseThrow(UserNotFoundException::new);
    }

    public boolean isExistUserById(Long userId) {
        return client.sql("SELECT EXISTS(SELECT 1 FROM users WHERE user_id = :user_id)").param("user_id", userId)
            .query(Boolean.class).optional().orElseThrow(UserNotFoundException::new);
    }
    ...
    @Override public Boolean isBanned(Long id) {
        Object result =
            client.sql("SELECT is_banned FROM Users WHERE user_id = :id").param("id", id).query().rowSet().first();
